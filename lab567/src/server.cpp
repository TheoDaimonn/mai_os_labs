#include "node_routine.hpp"
#include "socket_routine.hpp"
#include <fstream>
#include <signal.h>
#include <map>
#include <chrono>

int main(int argc, char **argv) {
    if (argc != 3) {
        perror("Not enough arguments");
        exit(EXIT_FAILURE);
    }
    
    Node task(atoi(argv[1]), atoi(argv[2]));
    std::string programPath = getenv("PROGRAM_PATH");

    // Добавляем структуры для таймера
    std::map<int, std::chrono::steady_clock::time_point> timers;
    std::map<int, std::chrono::milliseconds> elapsedTimes;

    while (1) {
        std::string message;
        std::string command = " ";
        if (auto msg = ReceiveMessage(&(task.parent)); msg.has_value()) {
            message = *msg;
        }
        std::istringstream request(message);
        request >> command;

        if (command == "create") {
            int idChild;
            request >> idChild;
            std::string ans = task.Create(idChild, programPath);
            SendMessage(&task.parent, ans);
        } else if (command == "pid") {
            std::string ans = task.Pid();
            SendMessage(&task.parent, ans);
        } else if (command == "ping") {
            int idChild;
            request >> idChild;
            std::string ans = task.Ping(idChild);
            SendMessage(&task.parent, ans);
        } else if (command == "send") {
            int id;
            request >> id;
            std::string str;
            getline(request, str);
            str.erase(0, 1);
            std::string ans;
            ans = task.Send(str, id);
            SendMessage(&task.parent, ans);
        } else if (command == "exec") {
    int id;
    std::string subcommand;
    request >> id >> subcommand;

    std::string response = "Ok:" + std::to_string(task.id);

    if (subcommand == "start") {
        timers[id] = std::chrono::steady_clock::now();
    } else if (subcommand == "stop") {
        if (timers.count(id)) {
            auto now = std::chrono::steady_clock::now();
            elapsedTimes[id] += std::chrono::duration_cast<std::chrono::milliseconds>(now - timers[id]);
            timers.erase(id); 
        }
    } else if (subcommand == "time") {
        if (elapsedTimes.count(id)) {
            response += ": " + std::to_string(elapsedTimes[id].count());
        } else {
            response += ": 0"; 
        }
    }        

            SendMessage(&task.parent, response);
        } else if (command == "kill") {
            std::string ans = task.Kill();
            ans = std::to_string(task.id) + " " + ans;
            SendMessage(&task.parent, ans);
            Disconnect(&task.parent, task.parentPort);
            task.parent.close();
            break;
        }
    }
    return 0;
}
